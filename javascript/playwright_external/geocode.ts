export async function geocode(city_name: string): Promise<[number, number]> {
    /**
     ...
     * @returns {Promise<[number, number] | null>} A promise that resolves to a tuple (latitude, longitude) if found, or [0,0].
     */
