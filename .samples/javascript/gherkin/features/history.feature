Feature: Account Transaction History
  ...
  # After start
    When I apply a date filter for "May 2024"
    Then I should see exactly 3 transactions
    And the displayed transactions should be:
      | Date       | Amount | Type   | Description       |
      | 2024-05-28 | 120.00 | Debit  | Restaurant Dinner |
      | 2024-05-25 | 500.00 | Credit | Freelance Payment |
      | 2024-05-20 | 30.00  | Debit  | Weekly Groceries  |
    But no transactions from "June 2024" or "April 2024" should be visible
  # After end

  Scenario: Filter transactions by type (Credits only)
  # After start
    When I apply a filter for transaction "Type" as "Credit"
    Then I should see exactly 3 transactions
    And the displayed transactions should be:
      | Date       | Amount | Type   | Description           |
      | 2024-06-14 | 1500.00| Credit | Monthly Salary Deposit|
      | 2024-05-25 | 500.00 | Credit | Freelance Payment     |
      | 2024-04-10 | 1000.00| Credit | Performance Bonus     |
    But no "Debit" transactions should be displayed
  # After end

  Scenario: Filter transactions by amount range ($50.00 to $150.00)
  # After start
    When I apply an amount filter between "$50.00" and "$150.00"
    Then I should see exactly 3 transactions
    And the displayed transactions should be:
      | Date       | Amount | Type  | Description         |
      | 2024-06-15 | 50.00  | Debit | Coffee Shop         |
      | 2024-06-12 | 75.00  | Debit | Streaming Service   |
      | 2024-05-28 | 120.00 | Debit | Restaurant Dinner   |
    But no transactions with amounts outside this range should be visible
    # After end
