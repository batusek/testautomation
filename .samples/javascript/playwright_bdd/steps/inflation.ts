...
When('I calculate the impact of inflation on my savings', async ( {world}) => {
  ...
  await world.page.getByRole('button', { name: 'cookies', exact: true }).click();
  await world.page.getByTestId('inflation-savings--input').fill(world.savings);
  await world.page.getByTestId('inflation-years--input').fill(world.years);
  await world.page.getByTestId('inflation-inflation-value--input').fill(world.inflation);
});

Then('value of my savings after all those years is {string}', async ( {world} , result) => {
  const actual = await world.page.getByTestId("resultAfterInflation");
